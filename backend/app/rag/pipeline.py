...
async def retrieve(
    ...
    q = (
        ...
        # by a newer version) stay eligible too, so conflict detection and
        # "what changed" comparisons can still see them - temporal_weight()
        # below heavily deprioritizes them rather than hiding them outright.
        .filter(
    ...
    # --- Keyword search (BM25) ---
    corpus = [_keyword_tokenize(chunk.content) for chunk, _doc in rows]
    bm25 = BM25Okapi(corpus) if corpus else None
    bm25_scores = bm25.get_scores(_keyword_tokenize(query)) if bm25 else [0.0] * len(rows)
    max_bm25 = max(bm25_scores) or 1.0

    candidates: List[RetrievedChunk] = []
    for (chunk, doc), bm25_score in zip(rows, bm25_scores):
        # --- Semantic search ---
        semantic = cosine_similarity(query_vector, chunk.embedding or [])
        keyword_norm = bm25_score / max_bm25 if max_bm25 else 0.0

        # Hybrid relevance: weighted blend of semantic + keyword signals
        relevance = 0.7 * semantic + 0.3 * keyword_norm
...
def _build_context(results: List[RetrievedChunk], conflicts: List[models.DocumentConflict]) -> str:
    ...
    for i, r in enumerate(results, start=1):
        parts.append(
            f"[Source {i}] \"{r.document.title}\" (page {r.chunk.page_number}, "
