...
export function Button({
  ...
  const variants: Record<string, string> = {
    primary: "bg-navy-700 text-paper-50 hover:bg-navy-900 active:bg-ink-950",
    secondary:
      "bg-surface text-ink-900 border border-line-strong hover:border-navy-700 hover:text-ink-950",
    ghost: "text-ink-700 hover:bg-paper-200",
    danger: "bg-seal-coral-600 text-white hover:bg-seal-coral-700",
  };
  return (
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className={`h-11 rounded-[var(--radius-control)] border px-3.5 text-sm bg-surface text-ink-900 placeholder:text-ink-400 outline-none transition-colors ${
          error ? "border-seal-coral-600" : "border-line-strong focus:border-navy-700"
        } ${className}`}
        {...rest}
      />
...
export function Select({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <select
        id={id}
        className={`h-11 rounded-[var(--radius-control)] border border-line-strong px-3.5 text-sm bg-surface text-ink-900 outline-none focus:border-navy-700 ${className}`}
        {...rest}
      >
        {children}
      </select>
